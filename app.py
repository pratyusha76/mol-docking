...
st.markdown("Automate receptor sanitization, ligand feature analysis, and grid-targeted compound docking in a seamless pipeline.")
st.divider()
...
if st.session_state.pdb_text:
    ...
    with c1:
        ...
        render_3d_viewer(st.session_state.pdb_text, style=render_mode, height=400)
        
    with c2:
        st.subheader("Pure Structure (Isolated Protein)")
        if st.session_state.pure_protein:
            render_3d_viewer(st.session_state.pure_protein, style=render_mode, height=400)
...
# Only allow interacting with Phase 2 if Phase 1 has some data, bridging the workflow.
if not st.session_state.pdb_text:
    ...
    with col_l1:
        ...
        if st.session_state.smiles and 'mol' in locals() and mol:
            ...
            st.image(img, caption="Chemical Structure Graph", use_column_width=False)
...
if not st.session_state.pdb_text or not st.session_state.smiles:
    ...
    if st.button("🚀 Initialize Molecular Docking Execution", type="primary"):
        ...
        for percent_complete in range(100):
            time.sleep(0.01)  # Simulating computational geometry scoring
        ...
        with res_c2:
            ...
            render_3d_viewer(st.session_state.pure_protein, ligand_smiles=st.session_state.smiles, style="cartoon", height=450)
